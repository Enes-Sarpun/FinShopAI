...
import {
  ...
  Sun, Moon, Pencil, Check, Star, Trash2, User, LifeBuoy,
} from "lucide-react";
...
function ThemeToggle({ collapsed }: { collapsed: boolean }) {
  ...
  const label = isDark ? t("settings.systemDark") : t("settings.systemLight");
  const nextLabel = isDark ? t("settings.systemNext") : t("settings.systemNextDark");

  return (
    <button
      onClick={toggle}
      title={nextLabel}
      className="flex items-center gap-3 w-full px-2.5 py-2 rounded-lg text-sm text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800 hover:text-gray-900 dark:hover:text-gray-100 transition-colors group"
    >
      <Icon className="w-4 h-4 flex-shrink-0" />
      {!collapsed && (
        <span className="flex-1 text-left truncate">{t("settings.system")}</span>
      )}
      {!collapsed && (
...
function SidebarContent({ collapsed, setCollapsed, userName, userEmail, history, onDeleteHistoryItem, onClose }: ContentProps) {
  ...
  const bottomItems = [
    { href: "/settings/account", icon: User, label: t("settings.myAccount") },
    { href: "/settings", icon: Settings, label: t("navigation.settings") },
    { href: "/support", icon: LifeBuoy, label: t("navigation.help") },
  ];
  ...
  return (
    <>
      ...
      <button
        ...
        className="lg:hidden fixed top-4 left-4 z-40 p-2 bg-white dark:bg-gray-900 rounded-xl shadow-md border border-gray-200 dark:border-gray-700"
      >
      ...
      <AnimatePresence>
        {mobileOpen && (
          <motion.aside
            ...
            className="lg:hidden fixed top-0 left-0 h-full w-64 bg-white dark:bg-gray-900 border-r border-gray-200/80 dark:border-gray-700/60 z-50 shadow-xl"
          >
            <SidebarContent
              collapsed={false}
              setCollapsed={() => {}}
      ...
      <motion.aside
        ...
        className="hidden lg:flex flex-col h-full bg-white dark:bg-gray-900 border-r border-gray-200/80 dark:border-gray-700/60 flex-shrink-0 overflow-hidden"
      >
        <SidebarContent
          collapsed={collapsed}
